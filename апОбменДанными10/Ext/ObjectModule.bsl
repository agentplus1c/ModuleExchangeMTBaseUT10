...
Перем гПлатформа83; 	   // sk_190830 Добавлена - Булево - Требуется для проверкии версии платформы
...
Процедура ИнициализироватьПеременные()
	...
	//(( sk_190906 Добавляем константу "ОтображатьОстаткиВРазрезеСкладов" в рамках (MOD-800)
	эл = ТаблЗначКонстанты.Добавить();		
	эл["Имя"] = "ПродажаОтображатьОстаткиВРазрезеСкладов";
	НастройкиАгентаПоУмолчанию["ПродажаОтображатьОстаткиВРазрезеСкладов"] = Истина; 
	эл["Ид"] = "c8d1d87e-fddb-4c24-bcca-89aa084e268c";
	эл["Представление"] = НСтр("ru='Отображать остатки товара по выбраному складу по умолчанию'"); 
	эл["Описание"] = НСтр("ru='В типовом приложении «Агент Плюс: Мобильная торговля» в версии 1.0.20.0 и выше, при включённой настройке, после выбора склада на вкладке «Основные» документа на МУ в окне подбора товара будет отображаться остаток только по выбранному складу.'");
	эл["Показывать"] = Истина;    
	эл["Тип"] = ТипКонстанты["Пользователь"];
	эл["Иерархия"] = НСтр("ru='Документы|Настройки для документов продаж'"); 
	эл["Выгружать"] = Истина;
	...
	//(( sk_190905 Добавляем настройку в рамках (MOD-782)
	эл = ТаблЗначКонстанты.Добавить();
	эл["Имя"] = "ВыгружатьДолгиТолькоПоДоговору";
	НастройкиОбработкиПоУмолчанию["ВыгружатьДолгиТолькоПоДоговору"] = Ложь;
	эл["Описание"] = НСтр("ru='Выгружать долги только по договору'");
	эл["Тип"] = ТипКонстанты["Обработка"];
	эл["Выгружать"] = Ложь;
	//)) sk_190905
...
// sk_190830 Функция добавлена
// Возвращает признак использования версии платформы 8.3.12 и выше
//
// Возвращаемое значение:
//	Булево - 
//			Истина - используется версия платформы 8.3.12 и выше 
//			Ложь   - используется версия платформы ниже 8.3.12
//
Функция Платформа83() Экспорт

	Если гПлатформа83 = Неопределено Тогда
		СистемнаяИнформация = Новый СистемнаяИнформация();
		ВерсияПлатформы = СистемнаяИнформация.ВерсияПриложения;
		
		Если СравнитьВерсии(ВерсияПлатформы, "8.3.12") >= 0  Тогда
			гПлатформа83 = Истина;
		Иначе
			гПлатформа83 = Ложь;
		КонецЕсли;
	...
	Возврат гПлатформа83;

КонецФункции // Платформа83()
...
Процедура ДобавитьСтрокуТабличнойЧастиВДокумент(ОбъектДок, ВидДокумента, стррСтрокаТЧ, стррДополнительныеПараметры = Неопределено)
	...
	//53. СтавкаНДС
	Если НЕ ВидДокумента = "ПеремещениеТоваров" Тогда
		//(( sk_190903 Изменено заполнение ставки НДС в табличной части в рамках (MOD-664) 
		// В КА 1.1.106.1 ОбработкаТабличныхЧастей.ЗаполнитьСтавкуНДСТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, ВидДокумента = "")
		// В УТ 10.3.55.2 ОбработкаТабличныхЧастей.ЗаполнитьСтавкуНДСТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, ВидДокумента = "")
		// В УТП для Казахстана 2.0.16.1 ОбработкаТабличныхЧастей.ЗаполнитьСтавкуНДСТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект)
		// НоваяСтрока.СтавкаНДС = ТекНоменклатура.СтавкаНДС;
		
		Если Не НаименованиеКонфигурации = "УправлениеТорговымПредприятиемДляКазахстана" 
			И (ВидДокумента = "ЗаказПокупателя" ИЛИ ВидДокумента = "РеализацияТоваровУслуг" ИЛИ ВидДокумента = "ВозвратТоваровОтПокупателя") Тогда   
			ОбработкаТабличныхЧастей.ЗаполнитьСтавкуНДСТабЧасти(НоваяСтрока, ОбъектДок, "Реализация");
		ИначеЕсли Не НаименованиеКонфигурации = "УправлениеТорговымПредприятиемДляКазахстана" И ВидДокумента = "ПоступлениеТоваровУслуг" Тогда
			ОбработкаТабличныхЧастей.ЗаполнитьСтавкуНДСТабЧасти(НоваяСтрока, ОбъектДок, "Приобретение");
		Иначе
	...
	//54. СуммаНДС
	Если НЕ ВидДокумента = "ПеремещениеТоваров" Тогда
		//(( sk_190903 Расчитываем сумму НДС в табличной части в рамках (MOD-664)
...
Процедура ВыгрузитьВзаиморасчеты(XML_Текст, сзКлиентыАгента)
	...
	//(( sk_190905 Изменен запрос в процедуре выгрузки в рамках (MOD-719)  
	// РН.ВзаиморасчетыСКонтрагентамиПоДокументамРасчетов 
	// Регистр используется в случае, если расчеты с Контрагентом идут по договору, с установленным признаком «Вести по документам расчетов с контрагентом».
	//
	// РН.ВзаиморасчетыСКонтрагентами
	// Регистр используется для ведения учета взаиморасчетов с контрагентами. В отличии от регистра «Взаиморасчеты с контрагентами по документам расчетов» 
	// сюда попадают все записи по взаиморасчетам, независимо от характеристик договора контрагента.
	// Регистр представляет собой суммовое отражение отношений с клиентами в разрезе договоров, организаций и сделок.
	//
	// Комментарий:
	// Таким образом, если в договоре с контрагентом стоит галочка «Вести по документам расчетов с контрагентом» записи по документу попадут в оба регистра, 
	// при условии, что галочка «Управленческий учет» в документе установлена. 
	//
	// Добавлен пересчет сумм долга в валюту регламентного учета.
    // Добавлен режим выгрузки сумм долга по договорам, без детализации по документам расчетов с контрагентами.
	...
	флВыгружатьДолгиТолькоПоДоговору = НастройкиОбработки["ВыгружатьДолгиТолькоПоДоговору"];
	
	МенеджерВТ = Новый МенеджерВременныхТаблиц;
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВТ;
	
	// 1. Заполняем вспомогательные таблицы ВТ_КатегорииДокументов и ВТ_КурсыВалют  
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	ЗначенияСвойствОбъектов.Значение КАК Категория,
	               |	ЗначенияСвойствОбъектов.Объект КАК Объект
	               |ПОМЕСТИТЬ ВТ_КатегорииДокументов
	               |ИЗ
	               |	РегистрСведений.ЗначенияСвойствОбъектов КАК ЗначенияСвойствОбъектов
	               |ГДЕ
	               |	ЗначенияСвойствОбъектов.Свойство = &АгентПлюсКатегория
	               |
	               |ИНДЕКСИРОВАТЬ ПО
	               |	Объект
	               |;
	               ...
	               |ИЗ
	               |	РегистрСведений.КурсыВалют.СрезПоследних(&ДатаКон, ) КАК КурсыВалют
	               |
	               |ИНДЕКСИРОВАТЬ ПО
	               |	Валюта";
	
	Запрос.УстановитьПараметр("АгентПлюсКатегория", ПолучитьСсылкуСвойствоОбъектаАгентПлюс("Категория"));
	Запрос.УстановитьПараметр("ВалютаУчета", 		мВалютаРегламентированногоУчета);
	Запрос.УстановитьПараметр("ДатаКон",     	 	ТекущаяДата());	
	Запрос.УстановитьПараметр("СписокКлиентов",  	сзКлиентыАгента);
	
	Запрос.Выполнить();
	
	// 2. Помещаем данные по долгам в таблицу ВТ_ДолгиБезГруппировки 
	Если Не флВыгружатьДолгиТолькоПоДоговору Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               |	ВзаиморасчетыСКонтрагентамиПоДокументамРасчетовОстатки.Организация КАК Организация,
		               |	ВзаиморасчетыСКонтрагентамиПоДокументамРасчетовОстатки.Контрагент КАК Контрагент,
		               ...
		               |	ВзаиморасчетыСКонтрагентамиПоДокументамРасчетовОстатки.ДокументРасчетовСКонтрагентом КАК ДокументРасчетовСКонтрагентом,
		               |	ВзаиморасчетыСКонтрагентамиПоДокументамРасчетовОстатки.СуммаВзаиморасчетовОстаток КАК СуммаВзаиморасчетовОстаток
		               |ПОМЕСТИТЬ ВТ_ДолгиБезГруппировки
		               |ИЗ
		               |	РегистрНакопления.ВзаиморасчетыСКонтрагентамиПоДокументамРасчетов.Остатки(
		               |			&ДатаКон,
		               |			УпрУчет = ИСТИНА
		               |				И Контрагент В (&СписокКлиентов)) КАК ВзаиморасчетыСКонтрагентамиПоДокументамРасчетовОстатки
		               |
		               |ОБЪЕДИНИТЬ ВСЕ
		               |
		               |ВЫБРАТЬ
		               ...
		               |	ВзаиморасчетыСКонтрагентамиОстатки.СуммаВзаиморасчетовОстаток
		               |ИЗ
		               |	РегистрНакопления.ВзаиморасчетыСКонтрагентами.Остатки(&ДатаКон, Контрагент В (&СписокКлиентов)) КАК ВзаиморасчетыСКонтрагентамиОстатки
		               |
		               |ИНДЕКСИРОВАТЬ ПО
	...
	// 3. Формируем таблицу по долгам ВТ_Долги 
	Запрос.Текст = "ВЫБРАТЬ
	               |	ВТ_ДолгиБезГруппировки.Организация КАК Организация,
	               |	ВТ_ДолгиБезГруппировки.Контрагент КАК Контрагент,
	               |	ВТ_ДолгиБезГруппировки.Договор КАК Договор,
	               |	ВТ_ДолгиБезГруппировки.Сделка КАК Сделка,
	               |	ВТ_ДолгиБезГруппировки.ДокументРасчетовСКонтрагентом КАК ДокументРасчетовСКонтрагентом,
	               |	СпрДоговорыКонтрагентов.ВалютаВзаиморасчетов КАК ВалютаВзаиморасчетов,
	               |	СУММА(ВТ_ДолгиБезГруппировки.СуммаВзаиморасчетовОстаток) КАК СуммаВзаиморасчетовОстаток
	               |ПОМЕСТИТЬ ВТ_Долги
	               |ИЗ
	               |	ВТ_ДолгиБезГруппировки КАК ВТ_ДолгиБезГруппировки
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК СпрДоговорыКонтрагентов
	               |		ПО ВТ_ДолгиБезГруппировки.Договор = СпрДоговорыКонтрагентов.Ссылка
	               |ГДЕ
	               |	ВТ_ДолгиБезГруппировки.СуммаВзаиморасчетовОстаток > 0
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ВТ_ДолгиБезГруппировки.Организация,
	               |	ВТ_ДолгиБезГруппировки.Контрагент,
	               |	ВТ_ДолгиБезГруппировки.Договор,
	               |	ВТ_ДолгиБезГруппировки.Сделка,
	               |	ВТ_ДолгиБезГруппировки.ДокументРасчетовСКонтрагентом,
	...
	// 4. Итоговый запрос с пересчетом сумм в валюту регламентного учета
	Запрос.Текст = "ВЫБРАТЬ
	               |	ВТ_Долги.Организация КАК Организация,
	               |	ВТ_Долги.Контрагент КАК Контрагент,
	               |	ВТ_Долги.Договор КАК Договор,
	               |	ВТ_Долги.Сделка КАК Сделка,
	               |	ВТ_Долги.ДокументРасчетовСКонтрагентом КАК ДокументРасчетовСКонтрагентом,
	               |	ВТ_Долги.ВалютаВзаиморасчетов КАК ВалютаВзаиморасчетов,
	               |	ВЫРАЗИТЬ(ВЫБОР
	               |			КОГДА &ВалютаУчета <> ВТ_Долги.ВалютаВзаиморасчетов
	               |				ТОГДА ВТ_Долги.СуммаВзаиморасчетовОстаток * ВТ_КурсыВалют.КурсКратный
	               |			ИНАЧЕ ВТ_Долги.СуммаВзаиморасчетовОстаток
	               |		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаВзаиморасчетовОстаток,
	               |	ВТ_КатегорииДокументов.Категория КАК Категория
	               |ИЗ
	               |	ВТ_Долги КАК ВТ_Долги
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_КурсыВалют КАК ВТ_КурсыВалют
	               |		ПО ВТ_Долги.ВалютаВзаиморасчетов = ВТ_КурсыВалют.Валюта
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_КатегорииДокументов КАК ВТ_КатегорииДокументов
	               |		ПО ВТ_Долги.Сделка = ВТ_КатегорииДокументов.Объект
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК СпрКонтрагенты
	               |		ПО ВТ_Долги.Контрагент = СпрКонтрагенты.Ссылка
	               |
	               |УПОРЯДОЧИТЬ ПО
	...
	Запрос.Текст = "УНИЧТОЖИТЬ ВТ_КатегорииДокументов
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               ...
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |УНИЧТОЖИТЬ ВТ_ДолгиБезГруппировки
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |УНИЧТОЖИТЬ ВТ_Долги";
	...
	ствРеквизиты = СоответствиеДок[СоответствиеДок["Долг"]];	
	стррДолг = Новый Структура("Организация, Контрагент, Договор, Долг, Категория");
	
	XML_Текст.ЗаписатьНачалоЭлемента(эл_Документ);		
	
		XML_Текст.ЗаписатьАтрибут(ат_Гуид, СоответствиеДок["Долг"]);
		
		XML_Текст.ЗаписатьАтрибут(ат_Комментарий, ствРеквизиты["_Представление"]);	
		...
		Пока Выборка.Следующий() Цикл
			...
			ДокументОснование = ?(ЗначениеЗаполнено(Выборка.ДокументРасчетовСКонтрагентом), Выборка.ДокументРасчетовСКонтрагентом, Выборка.Сделка);
			Договор = Выборка.Договор; 
			
			стррДолг.Организация = Выборка.Организация;
			стррДолг.Контрагент  = Выборка.Контрагент;
			стррДолг.Договор 	 = Договор;
			стррДолг.Долг 		 = Выборка.СуммаВзаиморасчетовОстаток;
			стррДолг.Категория   = Выборка.Категория;
						
			Если ЗначениеЗаполнено(ДокументОснование) Тогда
				ВыгрузитьДокументВзаиморасчетов(XML_Текст, ДокументОснование, стррДолг);	
			ИначеЕсли Договор.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоДоговоруВЦелом И Договор.ВестиПоДокументамРасчетовСКонтрагентом = Ложь
				Или флВыгружатьДолгиТолькоПоДоговору Тогда 
				ВыгрузитьДокументВзаиморасчетов(XML_Текст, Договор, стррДолг);
			КонецЕсли;
...
//ВерсияМодуля = "2.0.6.8";     // sk_190903 Изменено заполнение ставки НДС в табличной части (MOD-664)
ВерсияМодуля = "2.0.6.9";       // sk_190909 Добавлена настройка "Выгружать долги только по договору", при ее активации выполняется выгрузка сумм долга по договорам, без детализации по документам расчетов с контрагентами (MOD-715). Исправлены ошибки при работе с виртуальными объектами на платформе 8.2 (MOD-695). Добавлена выгрузка константы "ОтображатьОстаткиВРазрезеСкладов" (MOD-800). Реализована поддержка онлайн карт на платформе 8.3.14. (MOD-802).
