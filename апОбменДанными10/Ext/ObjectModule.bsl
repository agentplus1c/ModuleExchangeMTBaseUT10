...
Перем НаименованиеКонфигурации;
...
Процедура ИнициализироватьПеременные()
	...
	списокЗнач.Добавить(1, НСтр("ru='Из списка контрагентов для агента'"));
...
Процедура ЗаполнитьРеквизитУчитыватьНДС(ОбъектДок)
	...
	Иначе
		...
		//(( sk_190214 Исправлена ошибка при загрузке документа на конфигурации УТ 10.3.35.2 (MOD-259) и конфигурации "Управление торговлей для Казахстана" (2.2.18.5)
...
Функция ВыполнитьОбновлениеДоНовойВерсии() Экспорт
	...
	Если ОбновитьДоВерсии("2.0.5.20", стррВерсии, ИзменитьНастройки) Тогда
...
Процедура ЗагрузитьДокументЗаказ(ТекстXML, стррПараметры, ЗагружаемыеОбъекты, ОшибкаЧтенияITEM)
	...
	//Попытка //vd_180829
		...
		СтруктураПараметровРасчетаСкидок.Вставить("ВидРеализации"                 , Перечисления.ВидыСкидок.Оптовая);
		СтруктураПараметровРасчетаСкидок.Вставить("СуммаДокумента"                , СуммаДокументаБезСкидок);
		СтруктураПараметровРасчетаСкидок.Вставить("Карта"                         , ОбъектДок.ДисконтнаяКарта);
		СтруктураПараметровРасчетаСкидок.Вставить("УчитыватьНДС"                  , ОбъектДок.УчитыватьНДС);                      
		СтруктураПараметровРасчетаСкидок.Вставить("СуммаВключаетНДС"              , ОбъектДок.СуммаВключаетНДС);
		СтруктураПараметровРасчетаСкидок.Вставить("ВалютаРегламентированногоУчета", мВалютаРегламентированногоУчета);
		СтруктураПараметровРасчетаСкидок.Вставить("УчетнаяПолитика"               , ОбъектДок.мУчетнаяПолитика);
		//}}vd_180829
	...
	Если ТекстXML.Имя = "TABLES" И ТекстXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
		...
		Если ВРег(СокрЛП(ТекстXML.ПолучитьАтрибут("GUID"))) = "A10221FA-5AC6-4E8C-80D8-D40D8DFC7779" Тогда
			...
			Если Константы.ИспользоватьСерииНоменклатуры.Получить() Тогда
				...
				// ОбъектДок.ЗаполнитьТабличныеЧастиПередПроведениемУпр();
				Если текНаименованиеКонфигурации = "УправлениеТорговлейДляКазахстана" Тогда
...
Процедура ЗагрузитьДокументПеремещение(ТекстXML, стррПараметры, ОшибкаЧтенияITEM);
	...
	//(( sk_190214 в конфигурации "Управление торговлей для Казахстана" (2.2.18.5) отсутствует реквизит НДСвСтоимостиТоваров  
	// ОбъектДок.НДСвСтоимостиТоваров = Перечисления.ДействиеНДСВСтоимостиТоваров.НеИзменять;
	Если Не текНаименованиеКонфигурации = "УправлениеТорговлейДляКазахстана" Тогда
		ОбъектДок.НДСвСтоимостиТоваров = Перечисления.ДействиеНДСВСтоимостиТоваров.НеИзменять;
	КонецЕсли;
	//)) sk_190214
...
Процедура ЗагрузитьДокументОрдер(ТекстXML, стррПараметры, ЗагружаемыеОбъекты)
	...
	Если СтрокаПлатеж.КурсВзаиморасчетов <> 0 И ОбъектДок.КратностьДокумента <> 0 Тогда
		...
		//(( sk_190214 Исправлена ошибка при запуске обработки на конфигурации "Управление торговлей для Казахстана" (2.2.18.5) (MOD-269)
		Если текНаименованиеКонфигурации = "УправлениеТорговлейДляКазахстана" Тогда 
...
Процедура ВыгрузитьДокументВзаиморасчетов(XML_Текст, Док, СтруктураДолга)
	...
	СтруктураПолей.Вставить("DocNumber", НомерДок);
	СтруктураПолей.Вставить(СтруктураРеквизитов["Категория"],	 		Категория);
	СтруктураПолей.Вставить(СтруктураРеквизитов["Организация"],	 		Организация);
	СтруктураПолей.Вставить(СтруктураРеквизитов["Контрагент"],	 		Контрагент);
	СтруктураПолей.Вставить(СтруктураРеквизитов["ТорговаяТочка"],		ТоргТочка);
	СтруктураПолей.Вставить(СтруктураРеквизитов["Договор"],	 			Договор);
	СтруктураПолей.Вставить(СтруктураРеквизитов["ДатаОплаты"],	 		СрокОплаты);
	СтруктураПолей.Вставить(СтруктураРеквизитов["Сумма"],	 			СуммаДолга);
	СтруктураПолей.Вставить(СтруктураРеквизитов["Комментарий"],			Комментарий);
	СтруктураПолей.Вставить(СтруктураРеквизитов["ДокументОснование"], 	Док);
	СтруктураПолей.Вставить(СтруктураРеквизитов["Выделять"], 	 ВыделятьДокументВМУ);				
	СтруктураПолей.Вставить(СтруктураРеквизитов["Представление"], 	 ПредставлениеДок);		
...
//	|	И Взаиморасчеты.Организация = &Организация
	...
	ТестРезультат = Запрос.Выполнить().Выгрузить();// vd_171109 Удалить
...
Процедура ВыгрузитьЗаказыПокупателей(XML_Текст, ДанныеАгента, ТзКатегорииЦен, СписНеНайденныхТиповЦен);
	...
	СтруктураРеквизитов = СоответствиеДок[СоответствиеДок["ЗаказПокупателя"]];
	структураТЧ = СтруктураРеквизитов[СтруктураРеквизитов["ТчТовары"]];
	...
	XML_Текст.ЗаписатьНачалоЭлемента(эл_Документ);		
		...
		XML_Текст.ЗаписатьНачалоЭлемента(эл_Элементы);
			...
			Пока ВыборкаДок.Следующий() Цикл  
				...
				СтруктураПолей.Вставить(СтруктураРеквизитов["Организация"],  	 Док.Организация);
				
				Если ИспользоватьТорговыеТочки Тогда
					Если ЭтоКонтрагент(Док.Контрагент) Тогда
						 СтруктураПолей.Вставить(СтруктураРеквизитов["Контрагент"],  	 Док.Контрагент);
					Иначе
						 СтруктураПолей.Вставить(СтруктураРеквизитов["Контрагент"],  	 Док.Контрагент.ГоловнойКонтрагент);
						 СтруктураПолей.Вставить(СтруктураРеквизитов["ТорговаяТочка"],   Док.Контрагент);
					 КонецЕсли;
				Иначе
					 СтруктураПолей.Вставить(СтруктураРеквизитов["Контрагент"],  	 Док.Контрагент);
				КонецЕсли;
				
				СтруктураПолей.Вставить(СтруктураРеквизитов["ДоговорКонтрагента"],   Док.ДоговорКонтрагента);
				Если Не ПустаяСтрока(КодТипаЦены) Тогда
					СтруктураПолей.Вставить(СтруктураРеквизитов["ТипЦен"],  	 Док.ТипЦен);
				КонецЕсли;
				СтруктураПолей.Вставить(СтруктураРеквизитов["СуммаДокумента"],  Док.СуммаДокумента);
				СтруктураПолей.Вставить(СтруктураРеквизитов["ОбщийВес"],  	 	ОбщийВес);
				СтруктураПолей.Вставить(СтруктураРеквизитов["ДатаОтгрузки"],  	Док.ДатаОтгрузки);
				СтруктураПолей.Вставить(СтруктураРеквизитов["Комментарий"],  	Док.Комментарий);
				СтруктураПолей.Вставить(СтруктураРеквизитов["Категория"],  	 	ВыборкаДок.Категория);
				...
				Если ЗаписьРег.Выбран() Тогда
					...
					Если координатыGPS <> Неопределено Тогда						
						...
						Если координатыGPS.Количество() >= 2 Тогда
							СтруктураПолей.Вставить(СтруктураРеквизитов["Широта"],  	 	координатыGPS[0]);
							СтруктураПолей.Вставить(СтруктураРеквизитов["Долгота"],  	 	координатыGPS[1]);
							Если координатыGPS.Количество() = 3 Тогда
								СтруктураПолей.Вставить(СтруктураРеквизитов["ДатаТочкиТрека"], координатыGPS[2]);
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
				...
				Если Док.Товары.Количество() + Док.Услуги.Количество() > 0 Тогда 
					...
					XML_Текст.ЗаписатьНачалоЭлемента(эл_ТабличныеЧасти);					
						...
						XML_Текст.ЗаписатьАтрибут(ат_Гуид, СтруктураРеквизитов["ТчТовары"]);
						
						Для Каждого СтрокаТаб Из Док.Товары	Цикл				
							СтруктураПолей = Новый Структура;
							СтруктураПолей.Вставить(ат_Гуид,  Новый УникальныйИдентификатор());
							СтруктураПолей.Вставить(ат_ИдетификаторДокумента, Док);
							СтруктураПолей.Вставить(структураТЧ["Номенклатура"],   СтрокаТаб.Номенклатура);
							СтруктураПолей.Вставить(структураТЧ["ХарактеристикаНоменклатуры"],   СтрокаТаб.ХарактеристикаНоменклатуры);
							СтруктураПолей.Вставить(структураТЧ["ЕдиницаИзмерения"],   СтрокаТаб.ЕдиницаИзмерения);
							СтруктураПолей.Вставить(структураТЧ["Количество"],   СтрокаТаб.Количество);
							СтруктураПолей.Вставить(структураТЧ["Цена"],   СтрокаТаб.Цена*СтрокаТаб.ЕдиницаИзмерения.Коэффициент);
							СтруктураПолей.Вставить(структураТЧ["Сумма"],   СтрокаТаб.Сумма);
							СтруктураПолей.Вставить(структураТЧ["СуммаНДС"],   СтрокаТаб.СуммаНДС);
							СтруктураПолей.Вставить(структураТЧ["Вес"],   СтрокаТаб.ЕдиницаИзмерения.Вес*СтрокаТаб.Количество);
							//++Ratmir 2017-11-18
							Попытка
								СтруктураПолей.Вставить(структураТЧ["ПроцентСкидкиНаценки"],   СтрокаТаб.ПроцентСкидкиНаценки + СтрокаТаб.ПроцентАвтоматическихСкидок);
							Исключение
							...
							Если НаименованиеКонфигурации = "УправлениеТорговымПредприятиемДляКазахстана" Тогда 
								СтруктураПолей.Вставить(структураТЧ["СтавкаНДС"], Вычислить("УчетНДСИАкциза.ПолучитьСтавкуНДС(СтрокаТаб.СтавкаНДС)"));
							Иначе
								СтруктураПолей.Вставить(структураТЧ["СтавкаНДС"], Вычислить("УчетНДС.ПолучитьСтавкуНДС(СтрокаТаб.СтавкаНДС)"));
							КонецЕсли;
							
							Если ТипЗнч(СтрокаТаб.Размещение) = Тип("СправочникСсылка.Склады") Тогда
								СтруктураПолей.Вставить(структураТЧ["Размещение"],  СтрокаТаб.Размещение);
							КонецЕсли;    
							ЗаписатьЭлементВДокументОбмена(XML_Текст, СтруктураПолей);
						КонецЦикла;
						...
						Для Каждого СтрокаТаб Из Док.Услуги	Цикл				
							...
							СтруктураПолей.Вставить(структураТЧ["Номенклатура"],   СтрокаТаб.Номенклатура);														
							СтруктураПолей.Вставить(структураТЧ["Количество"],   СтрокаТаб.Количество);
							СтруктураПолей.Вставить(структураТЧ["Цена"],   СтрокаТаб.Цена);
							СтруктураПолей.Вставить(структураТЧ["Сумма"],   СтрокаТаб.Сумма);
							СтруктураПолей.Вставить(структураТЧ["СуммаНДС"],   СтрокаТаб.СуммаНДС);							
							//++Ratmir 2017-11-18
							Попытка
...
Процедура ВыгрузитьПродажи(XML_Текст, ДанныеАгента, ТзКатегорииЦен, СписНеНайденныхТиповЦен)
	...
	СтруктураРеквизитов = СоответствиеДок[СоответствиеДок["РеализацияТоваровУслуг"]];
	структураТЧ = СтруктураРеквизитов[СтруктураРеквизитов["ТчТовары"]];
		...
		XML_Текст.ЗаписатьНачалоЭлемента(эл_Элементы);
			...
			Пока ВыборкаДок.Следующий() Цикл  
				...
				Если ИспользоватьТорговыеТочки Тогда					
					Если ЭтоКонтрагент(Док.Контрагент) Тогда
						СтруктураПолей.Вставить(СтруктураРеквизитов["Контрагент"], 	  	Док.Контрагент);
						СтруктураПолей.Вставить(СтруктураРеквизитов["ТорговаяТочка"],   "");
					Иначе
			        	СтруктураПолей.Вставить(СтруктураРеквизитов["ТорговаяТочка"],   Док.Контрагент);						
						СтруктураПолей.Вставить(СтруктураРеквизитов["Контрагент"],		Док.Контрагент.ГоловнойКонтрагент);						
					КонецЕсли;
				...
				Если ЗначениеЗаполнено(Док.Сделка) И ТипЗнч(Док.Сделка) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
					СтруктураПолей.Вставить(СтруктураРеквизитов["Сделка"], 		Док.Сделка);
				КонецЕсли;
				...
				Если Док.Товары.Количество() + Док.Услуги.Количество() > 0 Тогда 
					...
					XML_Текст.ЗаписатьНачалоЭлемента(эл_ТабличныеЧасти);
						...
						Для Каждого СтрокаТаб Из Док.Товары	Цикл							
							СтруктураПолей = Новый Структура;
							СтруктураПолей.Вставить(ат_Гуид,  									Новый УникальныйИдентификатор());
							СтруктураПолей.Вставить(ат_ТчДокументРодитель, 						Док.Ссылка);
							СтруктураПолей.Вставить(структураТЧ["Номенклатура"],   				СтрокаТаб.Номенклатура);
							СтруктураПолей.Вставить(структураТЧ["ХарактеристикаНоменклатуры"],	СтрокаТаб.ХарактеристикаНоменклатуры);
							СтруктураПолей.Вставить(структураТЧ["ЕдиницаИзмерения"],  			СтрокаТаб.ЕдиницаИзмерения);
							СтруктураПолей.Вставить(структураТЧ["Количество"],   				СтрокаТаб.Количество);							
							...
							СтруктураПолей.Вставить(структураТЧ["Сумма"],   					СтрокаТаб.Сумма);
							СтруктураПолей.Вставить(структураТЧ["ПроцентСкидкиНаценки"], 		СтрокаТаб.ПроцентСкидкиНаценки);
							СтруктураПолей.Вставить(структураТЧ["СуммаСкидки"],  				(СтрокаТаб.Количество*СтрокаТаб.Цена) - СтрокаТаб.Сумма);
							СтруктураПолей.Вставить(структураТЧ["СуммаНДС"],					СтрокаТаб.СуммаНДС);
							СтруктураПолей.Вставить(структураТЧ["Вес"],   						СтрокаТаб.ЕдиницаИзмерения.Вес*СтрокаТаб.Количество);
							//СтруктураПолей.Вставить(структураТЧ["СтавкаНДС"],					УчетНДС.ПолучитьСтавкуНДС(СтрокаТаб.СтавкаНДС));
							...
							Если ТипЗнч(СтрокаТаб.Склад) = Тип("СправочникСсылка.Склады") Тогда
								СтруктураПолей.Вставить(структураТЧ["Склад"],  СтрокаТаб.Склад);
							Иначе
								СтруктураПолей.Вставить(структураТЧ["Склад"],  "");
							КонецЕсли;    
							ЗаписатьЭлементВДокументОбмена(XML_Текст, СтруктураПолей);
						КонецЦикла;
						...
						Для Каждого СтрокаТаб Из Док.Услуги	Цикл				
							СтруктураПолей = Новый Структура;
							СтруктураПолей.Вставить(ат_Гуид,  								Новый УникальныйИдентификатор());
							СтруктураПолей.Вставить(ат_ТчДокументРодитель,					Док);
							СтруктураПолей.Вставить(структураТЧ["Номенклатура"],			СтрокаТаб.Номенклатура);														
							СтруктураПолей.Вставить(структураТЧ["Количество"],  			СтрокаТаб.Количество);
							СтруктураПолей.Вставить(структураТЧ["Цена"],					СтрокаТаб.Цена);
							СтруктураПолей.Вставить(структураТЧ["Сумма"],					СтрокаТаб.Сумма);
...
Функция ПроверкаСозданияНастройкиСтатистикаОбратнаяСвязь() Экспорт
...
Функция ПолучитьНастройкиСтатистикаОбратнаяСвязь() Экспорт
	Возврат ХранилищеСистемныхНастроек.Загрузить("Статистика","Данные",,Неопределено);	
КонецФункции

Функция ЗаписатьНастройкиСтатистикаОбратнаяСвязь(Организация, ФИО, Номер, Флаг, Дата,Почта,Статус) Экспорт
	Попытка
		Данные = Новый Соответствие;
...
Функция ПолучитьСтруктуруДокумента(ВидДокумента) Экспорт

 	стррРезультат = Новый Структура;
	
	стррРезультат.Вставить("GUID");
	стррРезультат.Вставить("Номер");
	стррРезультат.Вставить("Дата");
	стррРезультат.Вставить("Контрагент");
	стррРезультат.Вставить("ПометкаУдаления");
	стррРезультат.Вставить("Проведен");
	
	стррТЧ = Новый Структура;
	
	стррТЧ.Вставить("GUID");
	стррТЧ.Вставить("Номер");
	стррТЧ.Вставить("Свойство");
	
	стррРезультат.Вставить("Товары", стррТЧ);
	
	Возврат стррРезультат;
	
КонецФункции // ПолучитьСтруктуруДокумента()

Функция ПолучитьОбъектДокументаИзХранилище(ВидДокумента, GUID) Экспорт

	стррДокумент = ПолучитьСтруктуруДокумента(ВидДокумента); 
	
	// ТЧ - Таблица значений
	
	Возврат стррДокумент; 

КонецФункции // ПолучитьОбъектДокумента()

Функция ПолучитьСписокДокументовТест(ВидДокумента) Экспорт

	тзСписок = Новый ТаблицаЗначений;
	
	тзСписок.Колонки.Добавить("GUID", Тип("УникальныйИдентификатор"));
	тзСписок.Колонки.Добавить("Номер", Тип("Строка"));
	тзСписок.Колонки.Добавить("Дата", Тип("Дата"));
	
	стзСтрока = тзСписок.Добавить();
	стзСтрока.GUID = Новый УникальныйИдентификатор();
	стзСтрока.Номер = "1";
	стзСтрока.Дата = ТекущаяДата();
	
	стзСтрока = тзСписок.Добавить();
	стзСтрока.GUID = Новый УникальныйИдентификатор();
	стзСтрока.Номер = "2";
	стзСтрока.Дата = ТекущаяДата() + 1;
	
	стзСтрока = тзСписок.Добавить();
	стзСтрока.GUID = Новый УникальныйИдентификатор();
	стзСтрока.Номер = "3";
	стзСтрока.Дата = ТекущаяДата() + 2;
...
ВерсияМодуля = "2.0.5.22";      // sk_190325 Исправлена выгрузка справочника "ВидыКонтактнойИнформации" на конфигурации "Комплексная автоматизация" в рамках (MOD-387).
